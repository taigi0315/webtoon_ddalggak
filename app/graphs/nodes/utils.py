...
def _assign_panel_weights(panel_plan: dict, scene_importance: str | None = None) -> dict:
    ...
    Heuristics:
    - Map utility_score -> weight (clamped 0.12 - 1.0)
    - If `scene_importance` is 'climax' or 'cliffhanger', prefer making reveal/reaction panels `must_be_large`.
    """
    panels = list(panel_plan.get("panels") or [])
    if not panels:
        return panel_plan

    # Utility scores expected in 0.0-1.0 range; derive weight from utility
    ...
    for p in panels:
        ...
        # normalize utility to 0.0-1.0 then scale to 0.12-1.0
        norm = (u - min_u) / range_u if range_u > 0 else 0.0
        weight = 0.12 + norm * (1.0 - 0.12)
        # Respect explicit provided weight if any (allow LLM override)
