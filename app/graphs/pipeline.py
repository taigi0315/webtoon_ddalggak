...
def _node_llm_scene_intent(state: PlanningState, gemini: GeminiClient | None) -> dict[str, Any]:
...
def _node_llm_panel_plan(state: PlanningState, gemini: GeminiClient | None) -> dict[str, Any]:
    with session_scope() as db:
...
def _node_rule_panel_plan_normalize(state: PlanningState) -> dict[str, Any]:
    with session_scope() as db:
        artifact = nodes.run_panel_plan_normalizer(db=db, scene_id=state["scene_id"])
    return {"panel_plan_normalized_artifact_id": str(artifact.artifact_id)}


def _node_rule_layout(state: PlanningState) -> dict[str, Any]:
    with session_scope() as db:
        artifact = nodes.run_layout_template_resolver(db=db, scene_id=state["scene_id"])
    return {"layout_template_artifact_id": str(artifact.artifact_id)}


def _node_llm_panel_semantics(state: PlanningState, gemini: GeminiClient | None) -> dict[str, Any]:
    with session_scope() as db:
...
def _node_qc(state: RenderState) -> dict[str, Any]:
    with session_scope() as db:
        artifact = nodes.run_qc_checker(db=db, scene_id=state["scene_id"])
...
def _node_render(state: RenderState, gemini: GeminiClient | None) -> dict[str, Any]:
    with session_scope() as db:
...
def build_scene_planning_graph(gemini: GeminiClient | None = None):
    graph = StateGraph(PlanningState)

    # LLM nodes
    graph.add_node("llm_scene_intent", partial(_node_llm_scene_intent, gemini=gemini))
    graph.add_node("llm_panel_plan", partial(_node_llm_panel_plan, gemini=gemini))
    graph.add_node("llm_panel_semantics", partial(_node_llm_panel_semantics, gemini=gemini))
...
def build_scene_render_graph(gemini: GeminiClient | None = None):
    ...
    graph.add_node("render", partial(_node_render, gemini=gemini))
...
def run_scene_planning(
    ...
    with track_graph_node("scene_planning", "run_scene_planning"):
        ...
        app = build_scene_planning_graph(gemini=gemini)
        state: PlanningState = {
            "scene_id": scene_id,
            "panel_count": panel_count,
            "genre": genre,
