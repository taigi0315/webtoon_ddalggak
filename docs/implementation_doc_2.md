...
#### Image generation config defaults

All image generation calls (scene render, ref images, rerenders) must use:

* `aspect_ratio = "9:16"` (enforced output aspect)
* `safety_settings` with all relevant categories set to `BLOCK_NONE`

Example (request config only):
...
* `POST /v1/stories/{id}/set-style-defaults`
...
* `environment_keywords`: list of environment tokens

How to change (examples):

* Allow more closeups → raise `closeup_ratio_max` (e.g., 0.6)
* Allow more dialogue → raise `dialogue_ratio_max` (e.g., 0.7)
* Be stricter on repeated framing → lower `repeated_framing_run_length` (e.g., 2)
* Disable environment requirement on establishing → set `require_environment_on_establishing` to false
* Add/remove environment hints → edit `environment_keywords`

Comparison notes (what changes):

* **Higher ratios** = fewer QC failures for dense closeups/dialogue
* **Lower ratios** = stricter QC, more fail-fast blocks
* **Shorter framing run length** = more “repeated framing” flags
* **Environment requirement off** = fewer early blocks, but higher risk of bland establishing panels
